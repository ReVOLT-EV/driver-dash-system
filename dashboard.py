...
class _DashBoardMain(QWidget):
    ...
    def __init__(self, parent, size: tuple | list = (1280, 720), skip_start_screen: bool = False, skip_loading_screen: bool = False, do_not_move: bool = False):
        super().__init__()
...
class _DashBoardControls(QObject):
    ...
    def launch_dashboard(self):
        ...
        self.dash_board = _DashBoardMain(None, (self.dashboard_width, self.dashboard_height), \
                                            self.start_skip, self.loading_skip)
        self.startup_values_setter()
        if __name__ == "__main__":  # for install default key event if dashboard called in main thread
            self.dash_board.installEventFilter(self.dash_board) 
        self.all_connector()
...
class DashBoard(QWidget):
    ...
    def show_dashboard(self, skip_start_screen: bool = False, skip_loading_screen: bool = False):
        """This method is to show the dashboard in your window"""
        global _dash_board
        
        self.dash_board_widget = _DashBoardMain(self, (self.width(), self.height()), skip_start_screen, skip_loading_screen, True)
